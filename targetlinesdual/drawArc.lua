...
local vertexData      = ffi.new('struct VertFormatFFFFUFF[200]');
...
local arcTex, orbTex;
local function drawArc(x1, y1, z1, x2, y2, z2, color, progress, orb, lineWidth)
    if (not helpers.isFinite(x1) or not helpers.isFinite(y1) or not helpers.isFinite(z1)
        or not helpers.isFinite(x2) or not helpers.isFinite(y2) or not helpers.isFinite(z2)
        or not helpers.isFinite(progress) or not helpers.isFinite(color)
        or color < 0 or color > 0xFFFFFFFF or (x1 == x2 and y1 == y2 and z1 == z2)
    ...
    if (world == nil or view == nil or projection == nil) then return; end
    ...
    if (not helpers.isFinite(wx0) or not helpers.isFinite(wy0) or not helpers.isFinite(wz0)
        or not helpers.isFinite(wx2) or not helpers.isFinite(wy2) or not helpers.isFinite(wz2)) then return; end
    ...
    if (not helpers.isFinite(P0x) or not helpers.isFinite(P0y) or not helpers.isFinite(P0z)
        or not helpers.isFinite(P1x) or not helpers.isFinite(P1y) or not helpers.isFinite(P1z)
        or not helpers.isFinite(P2x) or not helpers.isFinite(P2y) or not helpers.isFinite(P2z)) then return; end
    ...
    if (tMax <= tMin or progress < tMin) then
        return;
    end
    ...
    if (not helpers.isFinite(lineWidth) or lineWidth <= 0) then return; end
    local t = 0;
    do
    ...
    -- All Lua allocations/conversions happen before taking the device buffer lock.
    for _, vertex in ipairs(vertices) do
        for i = 1, 4 do if (not helpers.isFinite(vertex[i])) then return; end end
    end
    if (P2z > P0z) then
        for i = 0, 81 do
            vertexData[i] = ffi.new('struct VertFormatFFFFUFF', vertices[82 - i]);
        end
    else
        for i = 0, 81 do
            vertexData[i] = ffi.new('struct VertFormatFFFFUFF', vertices[i + 1]);
        end
    end
    ...
    if (vertexBuffer == nil) then return; end
    local result, ptr = vertexBuffer:Lock(0, 0, 0);
    if (result ~= C.S_OK) then return; end
    if (ptr == nil) then vertexBuffer:Unlock(); return; end
    local ok = pcall(ffi.copy, ptr, vertexData, (orb and 86 or 82) * ffi.sizeof(vertFormat));
    local unlocked = vertexBuffer:Unlock();
    if (not ok or unlocked ~= C.S_OK) then return; end
    arcTex = arcTex or getTexture(addon.path .. 'assets/beam.png');
    if (arcTex == nil) then return; end
    ...
    if (orb) then
        ...
        if (orbTex == nil) then return; end
        d3d8dev:SetTexture(0, orbTex);
        d3d8dev:DrawPrimitive(C.D3DPT_TRIANGLESTRIP, 82, 2);
    end
end

ashita.events.register('unload', 'targetlinesdual_release_cb', function()
    if (vertexBuffer ~= nil) then vertexBuffer:Release(); vertexBuffer = nil; end
    if (arcTex ~= nil) then ffi.gc(arcTex, nil); arcTex:Release(); arcTex = nil; end
    if (orbTex ~= nil) then ffi.gc(orbTex, nil); orbTex:Release(); orbTex = nil; end
end);
